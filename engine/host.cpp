...
//-----------------------------------------------------------------------------
// Sends a warning into the console. Tried to recode the function
//-----------------------------------------------------------------------------
void GMOD_SendError(std::string reason, std::string workshopid)
{
	...
	/*
		LUA->PushSpecial( GarrysMod::Lua::SPECIAL_GLOB );
			...
			If Table exists { // IDK how to check if a table exists
				LUA->GetField( -1, "Call" );
				
				if Function exists {
					LUA->PushString("LoadGModSaveFailed");
					LUA->PushNil();
					...
					Msg("gm_load: %s\n", "Couldn't find _G.hook.Call");
					LUA->Pop(2);
				}
			} else {
				Msg("gm_load: %s\n", "Couldn't find _G.hook");
				LUA->Pop(1);
			}
	*/
}
...
//-----------------------------------------------------------------------------
// Probably called when gm_load succeeded.
//-----------------------------------------------------------------------------
CUtlBuffer* g_GMLoadBuffer;
void GMOD_FinishLoading()
{
	/*
	LUA->PushSpecial( GarrysMod::Lua::SPECIAL_GLOB );
		LUA->GetField( -1, "hook" );
		...
		If Table exists { // IDK how to check if a table exists
			...
			if Function exists {
				...
				Msg("gm_load: %s\n", "Couldn't find Lua.hook.Call");
				LUA->Pop(2);
			}
		} else {
			Msg("gm_load: %s\n", "Couldn't find Lua.hook");
			LUA->Pop(1);
		}
*/
}

void gm_load_f(const CCommand& args)
{
	if (args.ArgC() < 1 || args.Arg(1) == "")
	{
		GMOD_SendError("Missing second argument!");
		return;
	}

	if (!g_pFullFileSystem->FileExists("save/" ..args.Arg(1), "GAME")) {
		GMOD_SendError("Error reading save from file!", "");
		return;
	}

	//if (SaveFile->ParseError) { // ToDo: Find out how it really works.
	//	GMOD_SendError("Error parsing save!", "");
	//	return;
	//}

	//if (!g_pFullFileSystem->FileExists("maps/[save map]", "GAME")) { // ToDo: Find out how to read a save file.
	//	GMOD_SendError("Unable to load the save because map file", "");
	//	return;
	//}
}

ConCommand my_function("gm_load", gm_load_f, "gm_load", FCVAR_GAMEDLL);